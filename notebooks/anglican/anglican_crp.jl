...
function anglican_crp_evaluate(results)
  ...
  samples = [samples; 1:anglican_crp_exact.K]
  weights = [weights; 1e-10*ones(anglican_crp_exact.K)]

  cluster_freq = zeros(Int64, anglican_crp_exact.K)
  map(c->cluster_freq[c]+=1, samples)
  println(cluster_freq)
  # println(samples)
  ...
  println("exact", anglican_crp_exact)
