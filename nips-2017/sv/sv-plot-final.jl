...
dfs = [load(TPATH*"/nips-2017/gmm/gmm-$i-df.jld")["df"] for i = ["0.5", "2.5"]]
...
vline_layer = layer(xintercept=[1], Geom.vline)

locs = [0.25, 0.75, 1.25, 1.75, 2.25, 2.75]
g_locs = locs - 0.05
...
ess_sv_p = plot(gibbs_layer_ess, nuts_layer_ess, vline_layer,
                Coord.cartesian(ymin = 0, xmin = 0), Guide.ylabel("ESS"), Guide.xlabel(nothing), Guide.xlabel(nothing),
...
# names = ["GMM 1", "GMM 2", "SV 1", "SV 2", "SV 3", "SV 4"]

time_sv_p = plot(gibbs_layer_time, nuts_layer_time, vline_layer,
                 Coord.cartesian(ymin = 0, xmin = 0, xmax = 3.05), Guide.ylabel("Time"), Guide.xlabel(nothing),
                 Guide.xticks(ticks=nothing),
                #  Scale.x_discrete(labels = i->names[i])
                #  Guide.manual_color_key("", ["NUTS","Gibbs"], colors[[2,1]])
                #  Guide.annotation(compose(context(), text(0.665, -2000, "x10"))),
                 Guide.annotation(compose(context(), text(0.4, 11800, "x10"))),
                #  xintercept=[5.0, 7.0], Geom.vline(style=[:solid,[1mm,1mm]])
                 )

vs = vstack(ess_sv_p, time_sv_p)

draw(PDF(TPATH*"/nips-2017/ess-time-final.pdf", 8inch, 3inch), vs)
...
g_locs = [0.25, 0.75] - 0.05
gibbs_layer_ess = layer(x = g_locs, xmin = g_locs - 0.05, xmax = g_locs + 0.05,
                       y = ess_mean_g, ymax = ess_maxs_g, ymin = ess_mins_g,
                       Geom.bar, Geom.errorbar, Theme(default_color = colors[1]))

n_locs = [0.25, 0.75] + 0.05
nuts_layer_ess = layer(x = n_locs, xmin = n_locs - 0.05, xmax = n_locs + 0.05,
                       y = ess_mean_n, ymax = ess_maxs_n, ymin = ess_mins_n,
                       Geom.bar, Geom.errorbar, Theme(default_color = colors[2]))

ess_gmm_p = plot(gibbs_layer_ess, nuts_layer_ess,
...
time_gmm_p = plot(gibbs_layer_time, nuts_layer_time,
                 Coord.cartesian(ymin = 0, xmin = 0, xmax = 1), Guide.ylabel(nothing), Guide.xlabel(nothing), Guide.xticks(label=false),
                #  Guide.manual_color_key("", ["NUTS","Gibbs"], colors[[2,1]])
                 )

vs2 = deepcopy(vstack(ess_gmm_p, time_gmm_p))

hs1 = hstack(ess_gmm_p, ess_sv_p)
hs2 = hstack(time_gmm_p, time_sv_p)

s = vstack(hs1, hs2)

draw(PDF(TPATH*"/nips-2017/gmm-ess-time-new.pdf", 8inch, 3inch), s)
