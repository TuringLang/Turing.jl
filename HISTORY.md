...
A Gibbs component may never change the dimension or the distributional form of a variable belonging to another component, unless the two share that variable. A component whose step changes which distribution another block's tilde statement draws from is refused, naming both components, where before it sampled silently: with `b` in one block and `x ~ Dirichlet(3)` under `b == 1` but `x ~ MvNormal(zeros(3), I)` otherwise, a draw from the second branch almost never lies on the simplex, so the chain could not return to `b == 1` and returned P(b=1) near 0 or near 1 against an exact 0.6456. Put the deciding variable and what it decides in one block, or write a sampler for them. What counts is the family together with the support, never the parameters, so `Normal(0, 1)` becoming `TDist(3)` is refused while an ordinary hierarchical `x ~ Normal(m, 1)` with `m` in another block is unaffected. The rule is deliberately conservative: whether a given support change is fatal is a question about irreducibility that no single model evaluation can decide, so partitions that would have sampled correctly are refused too, `x ~ truncated(Normal(); lower=a)` with `a` in another block among them. That includes the model from Turing.jl#2801, `x ~ Uniform(-5, 5); y ~ truncated(Normal(); lower=x)` under `Gibbs(:x => MH(), :y => HMC(0.1, 20))`, which sampled correctly and was tested against a NUTS ground truth: it is now refused, and the same model in one block, `Gibbs((@varname(x), @varname(y)) => MH())`, gives the same answer.

A Gibbs component can now be handed its block at a new shape between its own steps. That happens when another component decides whether one of the block's variables exists, as in `Gibbs((@varname(b), @varname(θ)) => PG(20), @varname(θ) => HMC(0.1, 5))`, and also when it moves one to a distribution that links differently, which can change the block's linked dimension while leaving its variables alone -- a simplex occupies one number fewer than the free vector of the same length. That previously failed with a `BoundsError` from inside the parameter layout. A sampler declares it copes by implementing the new five-argument form `Turing.Inference.gibbs_update_state!!(sampler, state, model, global_vals, ::Turing.Inference.ReshapedBlock)`, which defaults to throwing with an explanation. `MH`, `PG`/`CSMC`, `GibbsConditional` and any `Hamiltonian` not adapting -- `HMC`, and `NUTS(0, δ)` or `HMCDA(0, δ, λ)` -- implement it. An adapting `NUTS` or `HMCDA`, `ESS` and `externalsampler` do not, each carrying something sized for the block: a mass matrix adapted to the old shape in the Hamiltonian case. A sampler written against the four-argument form alone keeps the safe answer.
