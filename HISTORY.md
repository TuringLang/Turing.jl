...
A Gibbs component can now be handed its block at a new shape between its own steps, which happens when another component decides whether one of the block's variables exists, as in `Gibbs((@varname(b), @varname(θ)) => PG(20), @varname(θ) => HMC(0.1, 5))`. That previously failed with a `BoundsError` from inside the parameter layout. A sampler declares it copes by implementing the new five-argument form `Turing.Inference.gibbs_update_state!!(sampler, state, model, global_vals, ::Turing.Inference.ReshapedBlock)`, which defaults to throwing with an explanation. `MH`, `PG`/`CSMC` and the static `HMC` implement it; `NUTS`, `HMCDA`, `ESS` and `externalsampler` do not, each carrying something sized for the block -- an adapted mass matrix in the Hamiltonian case. A sampler written against the four-argument form alone keeps the safe answer.
