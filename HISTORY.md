...
A Gibbs component may never change the dimension of a variable belonging to another component, or whether that variable exists at all, unless the two share it. Put the deciding variable and what it decides in one block, or write a sampler for them. A variable deciding how many of another exist is the shape this catches: `b ~ Bernoulli(0.5); θ = Vector(undef, b ? 2 : 1); for i in eachindex(θ); θ[i] ~ Normal(); end` split as `Gibbs(:b => MH(), :θ => MH())` is refused, and `Gibbs((:b, :θ) => PG(20))` samples it correctly. The check observes only the states a component accepted, so a crossing proposed and rejected passes unnoticed; it refused that partition on all 40 seeds tried, but treat it as a net for mistakes rather than a guarantee.

A component moving another block's *support or distributional form* is permitted, and is your responsibility rather than Gibbs's. Each component's kernel remains invariant for its full conditional; what such a dependence can cost is irreducibility, and that turns on whether every reachable pair of supports overlaps, which no single model evaluation can decide. When they do not overlap the chain is absorbed and the answer is wrong with nothing raised: `b ~ Bernoulli(0.5)` choosing between `x ~ Uniform(0, 1)` and `x ~ Uniform(2, 3)`, split across two blocks, returned P(b=1) of 0.0, 0.0 and 1.0 over three seeds against 0.515 from the same model in one block. Neither dimension nor family distinguishes the safe case from the fatal one — that example holds both constant — so see the warning in the `Gibbs` docstring before relying on such a partition. Nested supports are the safe shape: `x ~ Uniform(-5, 5); y ~ truncated(Normal(); lower=x)` split across blocks explores the whole of `x`'s range and converges to the same answer as one block over both, and its regression test (Turing.jl#2801) asserts that again. Expect much slower mixing than the shared block: that split needed of the order of 200,000 draws to settle.
