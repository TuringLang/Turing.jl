...
This begs the question: how can these functions access model information during sampling? Recall that the model is stored as an instance `m` of `Model`. One of the attributes of `m` is the model evaluation function `m.f`: executing `f` runs the steps of the model in order and adds information to the sampler (the instance of `Sampler` that stores information about the ongoing sampling process) for every tilde statement.

Compiling the `@model` macro defines a function to instance of Model. main attribute of Model is the `f`, the model function. a large part of building the Model object is building this function from the lines in the @macro statement. 

among the arguments of `f` there is a sampler. this is because . how it modifies the sampler is defined by the assume and observe functions.

Given an instance `m` of `Model` and a sampler `spl`:

* `step!` will call `m(..., spl, ...)` which will call basically `m.f(..., spl, ...)`
