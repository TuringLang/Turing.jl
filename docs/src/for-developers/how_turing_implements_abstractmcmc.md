...
Consider an instance `m` of `Model` and a sampler `spl`. At some point during the sampling process, an AbstractMCMC function such as `step!` will call  `m(..., spl, ...)` which call the model evaluation function `m.f(..., spl, ...)`.
