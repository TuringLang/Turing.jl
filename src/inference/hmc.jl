...
function HMCState(model::Model, spl::Sampler{<:StaticHamiltonian}; kwargs...)
    ...
    h = AHMC.Hamiltonian(spl.alg.metricT(
        length(vi[spl])),
        logπ, ∂logπ∂θ)
...
function HMCDA{AD}(
    ...
    return HMCDA{AD, Any}(0, δ, λ, Set(), init_ϵ, metricT)
end
...
function HMCDA{AD}(
    ...
    return HMCDA{AD, Any}(n_adapts, δ, λ, Set(), init_ϵ, metricT)
end
...
function HMCDA{AD}(
    ...
    _space = isa(space, Symbol) ? Set([space]) : Set(space)
    return HMCDA{AD, eltype(_space)}(n_adapts, δ, λ, _space, init_ϵ, metricT)
end

"""
    NUTS(n_adapts::Int, δ::Float64)
...
function NUTS{AD}(
    ...
    _space = isa(space, Symbol) ? Set([space]) : Set(space)
    NUTS{AD, eltype(_space)}(n_adapts, δ, _space, max_depth, Δ_max, init_ϵ, metricT)
end
...
function NUTS{AD}(
    ...
    NUTS{AD, Any}(0, δ, Set(), max_depth, Δ_max, init_ϵ, metricT)
end
