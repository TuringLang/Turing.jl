...
function verifyspace(subalgs::Tuple{Vararg{InferenceAlgorithm}}, pvars, alg_str)
    ...
    _verifyspace(space, pvars, alg_str)
end
for T in (Tuple{}, Tuple{Vararg{Symbol}})
    @eval verifyspace(space::$T, pvars, alg_str) = _verifyspace(space, pvars, alg_str)
end
function _verifyspace(space, pvars, alg_str)
    if length(space) > 0
