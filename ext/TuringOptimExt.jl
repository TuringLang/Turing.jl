...
function StatsBase.informationmatrix(m::ModeResult; hessian_function=ForwardDiff.hessian, kwargs...)
    ...
    if linked
        Accessors.@set! m.f.varinfo = DynamicPPL.invlink!!(m.f.varinfo, m.f.model)
    end
    ...
    if linked
        Accessors.@set! m.f.varinfo = DynamicPPL.link!!(m.f.varinfo, m.f.model)
    end
...
function _optimize(
    ...
    Accessors.@set! f.varinfo = DynamicPPL.unflatten(f.varinfo, init_vals)
    Accessors.@set! f.varinfo = DynamicPPL.link!!(f.varinfo, model)
    init_vals = DynamicPPL.getparams(f)
    ...
    Accessors.@set! f.varinfo = DynamicPPL.unflatten(f.varinfo, M.minimizer)
    Accessors.@set! f.varinfo = DynamicPPL.invlink!!(f.varinfo, model)
