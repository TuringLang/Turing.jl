...
if isdefined(Base, :get_extension)
    import Turing
    import Turing: Distributions, DynamicPPL, ForwardDiff, NamedArrays, Printf, BangBang, Statistics, StatsAPI, StatsBase 
    import Optim
else
    import ..Turing
    import ..Turing: Distributions, DynamicPPL, ForwardDiff, NamedArrays, Printf, BangBang, Statistics, StatsAPI, StatsBase
...
function StatsBase.informationmatrix(m::ModeResult; hessian_function=ForwardDiff.hessian, kwargs...)
    ...
    if linked
        BangBang.@set!! m.f.varinfo = DynamicPPL.invlink!!(m.f.varinfo, m.f.model)
    end
    ...
    if linked
        BangBang.@set!! m.f.varinfo = DynamicPPL.link!!(m.f.varinfo, m.f.model)
    end
...
function _optimize(
    ...
    BangBang.@set!! f.varinfo = DynamicPPL.unflatten(f.varinfo, init_vals)
    BangBang.@set!! f.varinfo = DynamicPPL.link!!(f.varinfo, model)
    init_vals = DynamicPPL.getparams(f)
    ...
    BangBang.@set!! f.varinfo = DynamicPPL.unflatten(f.varinfo, M.minimizer)
    BangBang.@set!! f.varinfo = DynamicPPL.invlink!!(f.varinfo, model)
