...
@test ForwardDiff.gradient(x::Vector -> hmcInverseGamma(2.0, 3.0)(x[1]), [1])[1] ≈ dualpart(pdf(ddIG, Dual(1, 1)))
gradient(ddIG, 1)

# Normal
ddN = dNormal(0, 1)
@test pdf(ddN, 1) ≈ realpart(pdf(ddN, Dual(1)))
@test ForwardDiff.gradient(x::Vector -> hmcNormal(0, 1)(x[1]), [1])[1] ≈ dualpart(pdf(ddN, Dual(1, 1)))
...
pdf(ddMN, [Dual(2, 1), Dual(1, 1)])

# Bernoulli
ddB = dBernoulli(0.3)
@test pdf(ddB, 1) ≈ realpart(pdf(ddB, Dual(1)))
@test ForwardDiff.gradient(x::Vector -> hmcBernoulli(0.3)(x[1]), [1])[1] ≈ dualpart(pdf(ddB, Dual(1, 1)))
