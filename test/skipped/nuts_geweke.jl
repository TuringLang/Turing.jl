...
function Gadfly.plot(qq::QQPair, elements::ElementOrFunction...)
    return Gadfly.plot(;
        x=qq.qx, y=qq.qy, Geom.point, Theme(; highlight_width=0px), elements...
    )
end

# Now some shorthand functions
qqplot(x, y, elements::ElementOrFunction...) = Gadfly.plot(qqbuild(x, y), elements...)
function qqnorm(x, elements::ElementOrFunction...)
    return qqplot(
        Normal(),
        x,
...
@model function gdemo_fw()
    ...
    # s = 1
    m ~ Normal(0, sqrt(s))
    return y ~ MvNormal([m; m; m], [sqrt(s) 0 0; 0 sqrt(s) 0; 0 0 sqrt(s)])
end
...
show(
    scatterplot(
        qqm.qx[51:(end - 50)],
        qqm.qy[51:(end - 50)];
        title="QQ plot for m (removing first and last 50 quantiles):",
        canvas=DotCanvas,
    ),
)
...
show(
    scatterplot(
        qqs.qx[51:(end - 50)],
        qqs.qy[51:(end - 50)];
        title="QQ plot for s (removing first and last 50 quantiles):",
    ),
)
